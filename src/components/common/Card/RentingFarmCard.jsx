...
const RentingFarmCard = ({ farm }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div
          className="w-full bg-center bg-cover rounded-tl-[16px] rounded-tr-[16px] aspect-[4/3] sm:h-[284px] sm:aspect-auto"
          style={{
            backgroundImage: `url('${farm.thumbnailUrl || placeholder}')`,
          }}
        />
      ...
      {/* 내용 */}
      <div className="flex flex-col gap-4 pb-4 pt-3 px-4 sm:px-6 w-full">
        {/* 제목 + 면적 */}
        <div className="flex items-start justify-between w-full">
          <div className="font-semibold text-lg sm:text-2xl text-black tracking-[-0.48px] leading-[1.3] truncate">
            {farm.title}
          </div>
          <div className="font-normal text-sm text-[#777777] tracking-[-0.42px] leading-[1.5] text-right w-8 h-[25px] flex items-center justify-center">
            {farm.size || '-'}㎡
        ...
        <div className="flex items-center gap-2">
          <LocationIcon />
          <div className="font-normal text-sm sm:text-base text-black tracking-[-0.48px] leading-[1.4] truncate">
            {farm.address}
          </div>
        </div>
        ...
        <div className="flex items-end justify-between w-full">
          <div className="flex items-end gap-2">
            <div className="flex items-end gap-1">
              <div className="font-semibold text-lg sm:text-2xl text-[#1aa752] tracking-[-0.48px] leading-[1.3]">
                {farm.price?.toLocaleString()}
              </div>
              <div className="font-normal text-base sm:text-xl text-black h-6 sm:h-8 flex items-center w-[18px]">
                원
              </div>
            </div>
            <div className="font-normal text-base sm:text-xl text-black h-6 sm:h-[33px] flex items-center w-[7px]">
              /
            </div>
            <div className="flex items-center gap-1 h-6 sm:h-8 text-base sm:text-xl text-black">
              <div className="font-semibold">{farm.rentalPeriod}</div>
              <div className="font-normal">일</div>
            </div>
          </div>
          ...
          <button
            onClick={handleDetailClick}
            className="hidden sm:flex items-center gap-1 font-normal text-sm sm:text-base text-[#777777]"
          >
            자세히 보기
            <ArrowIcon />
          </button>
      ...
      <div className="px-4 sm:px-6 pb-4">
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/plant/${farm.id}`, { state: { openReviewModal: true } });
          }}
        >
          리뷰 작성
        </Button>
